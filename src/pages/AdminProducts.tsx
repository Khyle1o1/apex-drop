...
import { Plus, Pencil, Trash2, ChevronDown, ChevronUp, X, PackageOpen, Tags } from 'lucide-react';
...
export default function AdminProducts() {
  ...
  const { data: products = [], isLoading } = useQuery<ProductRow[]>({
    ...
    // Keep data warm so navigating back to Products is instant.
    staleTime: 60_000,
  ...
  const deleteCategoryMut = useMutation({
    mutationFn: (id: string) => apiJson<void>(`/api/admin/categories/${id}`, { method: 'DELETE' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-categories'] });
  ...
  return (
    ...
    >
      <div className="px-0 md:px-1 py-1 md:py-2 w-full min-w-0">
        {/* Top bar */}
        <div className="flex flex-wrap items-center justify-between gap-3 w-full mb-3 sm:mb-4">
        ...
        {/* Tabs */}
        <div className="mb-4 border-b border-border flex items-center gap-4">
          <button
            type="button"
        ...
        {/* Categories tab */}
        {activeTab === 'categories' && (
          <section className="mt-2 grid gap-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1.2fr)]">
            {/* Add category */}
            <div className="w-full rounded-xl border border-border bg-background p-4 space-y-3">
              ...
              >
                <Plus className="h-3.5 w-3.5" />
                {createCategoryMut.isPending ? 'Adding…' : 'Add Category'}
              </Button>
            </div>
            ...
            <div className="w-full rounded-xl border border-border bg-background p-4">
              ...
              <p className="mt-3 text-[11px] text-muted-foreground">
                Categories are selected inside the Category dropdown when adding or editing products.
              </p>
            </div>
          </section>
        )}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Delete <strong>{deleteCategoryTarget?.name}</strong>? Products in this category will
              have their category cleared but will not be deleted.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleteCategoryMut.isPending}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() =>
                deleteCategoryTarget && deleteCategoryMut.mutate(deleteCategoryTarget.id)
              }
              disabled={deleteCategoryMut.isPending}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
            >
              {deleteCategoryMut.isPending ? 'Deleting…' : 'Delete'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

...
export async function getOrCreateCart(userId: string) {
  ...
  if (!cart) throw new AppError("Failed to get cart", 500, "CART_ERROR");
...
export async function applyPromo(userId: string, code: string) {
  ...
  const [promo] = await db
    ...
    .limit(1);
  if (!promo) throw new AppError("Invalid or inactive promo code", 400, "INVALID_PROMO");
  const now = new Date();

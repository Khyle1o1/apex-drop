...
export async function register(body: RegisterBody): Promise<{ user: AuthUser }> {
  ...
  const [inserted] = await db
    ...
    .returning();
  if (!inserted) throw new AppError("Failed to create user", 500, "CREATE_FAILED");
  return {
...
export async function login(
  ...
  if (!ok) throw new AppError("Invalid identifier or password", 401, "INVALID_CREDENTIALS");
...
function toAuthUser(row: { id: string; email: string; idNumber: string; fullName: string; role: string; isActive: boolean }): AuthUser {
  return {
    id: row.id,
    email: row.email,
